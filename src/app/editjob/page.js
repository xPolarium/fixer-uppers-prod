  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const jobResponse = await fetch(`/api/jobs/${window.location.pathname.split('/').pop()}`);
      ...
      <Box sx={{ maxWidth: 800, mx: 'auto', my: 4, p: 2 }}>
        <Card sx={{ p: 3 }}>
          ...
          <Stack direction="row" justifyContent="space-between" sx={{ mb: 2 }}>
            ...
            <Typography level="title-lg">${formData.offered_price}</Typography>
          </Stack>
          ...
          <Stack direction="row" spacing={2} sx={{ mb: 3 }}>
            ...
            <Typography>Posted: {new Date(formData.timestamp).toLocaleDateString()}</Typography>
          </Stack>
